...
Model 8 (St_K_P) seems to be the best according to LOOIC and DIC values. Pareto k statistics look okay also...

```{r}
best.model.cliff.looic <- readRDS(paste0("RData/Richards_Cliff_", 
                                        model.names[[8]], "_norm_LOOIC.rds"))
...
```{r}
best.model.cliff <- readRDS(paste0("RData/Richards_Heat_", model.names[[8]], "_norm.rds"))

X_Cliff_St_K_P.df <- compute.summary.stats_St_K_P(best.model.cliff$samples, 
                                                  seasons, 
                                                  max.days$total.days)
