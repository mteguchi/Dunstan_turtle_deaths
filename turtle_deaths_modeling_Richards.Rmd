...
Model 4 (St_K1_K2) seems to be the best according to DIC values. Let's look at LOOIC values.

```{r}

M.Heat <- list(M1.Heat, M2.Heat, M3.Heat, M4.Heat, M5.Heat, M6.Heat)

loo.out <- list(length = length(M.Heat))
  
for (k in 1:length(M.Heat)){
...
```{r}
X_Heat_St_K.df <- compute.summary.stats_St_K(M2.Heat$samples, 
                                               seasons, 
                                               max.days$total.days, 
                                               P)
...
#mcmc_trace(M1.Heat$samples, c("sigma.y", "sigma.max"))

```


```{r}
X_Heat_S_K.df <- compute.summary.stats_S_K(samples = M1.Heat$samples, 
                                           seasons = seasons, 
