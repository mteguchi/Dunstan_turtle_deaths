...
```{r}
best.model.heat <- readRDS(paste0("RData/Richards_Heat_", model.names[[4]], "_norm.rds"))

X_Heat_St_K1_K2.df <- compute.summary.stats_St_K1_K2(best.model.heat$samples, 
                                                     seasons, 
                                                     max.days$total.days, 
                                                     P)
...
Model 6 (S1_S2_K1_K2) and Model 4 (St_K1_K2) seem to be the best according to LOOIC and DIC values, respectively. Pareto k statistics look okay also...

```{r}
best.model.cliff.looic <- readRDS(paste0("RData/Richards_Cliff_", 
                                        model.names[[6]], "_norm_LOOIC.rds"))
...
```{r}
good.model.cliff.looic <- readRDS(paste0("RData/Richards_Cliff_", 
                                        model.names[[4]], "_norm_LOOIC.rds"))

pareto_k_table(good.model.cliff.looic)
    
```

Less bad Pareto k diagnostic values for Model 6. So, we'll go with Model 6.

We'll use Model 6 for computing the total deaths due to the heat related reasons.

```{r}
best.model.cliff <- readRDS(paste0("RData/Richards_Heat_", model.names[[6]], "_norm.rds"))

X_Cliff_S1_S2_K1_K2.df <- compute.summary.stats_S1_S2_K1_K2(best.model.cliff$samples, 
                                                     seasons, 
                                                     max.days$total.days, 
                                                     P)

ggplot() + 
  geom_path(data = X_Cliff_S1_S2_K1_K2.df,
            aes(x = DOS, y = Med)) +
  geom_ribbon(data = X_Cliff_S1_S2_K1_K2.df,
              aes(x = DOS, ymin = Low, ymax = High),
              alpha = 0.4) + 
  geom_point(data = data.1,
             aes(x = DOS, y = Cliff)) + 
