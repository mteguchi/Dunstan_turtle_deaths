...
data.1 %>% select(Date, Heat, Season, DOS) %>%
  mutate(cause = "Heat",
         count = Heat) -> data.heat
data.1 %>% select(Date, Cliff, Season, DOS) %>%
  mutate(cause = "Cliff",
         count = Cliff) -> data.cliff

data.heat.wide <- dcast(data.heat, DOS ~ Season, value.var = "Heat")
data.cliff.wide <- dcast(data.cliff, DOS ~ Season, value.var = "Cliff")
...
data.0 %>% left_join(data.heat.wide, by = "Date") -> data.0.heat

data.2 %>% select(Season, DOSeason, `2014R`, `2017R`, North, South, West) -> data.2.a

obs.y <- array(NA, c(length(seasons), n.days))
...
if (!file.exists(paste0("RData/Fourier_X0_imputation_dnorm_", Day1, "_", Day2, ".rds"))){
  jm <- jags(jags.data,
             inits = NULL,
  ...
  saveRDS(jm, paste0("RData/Fourier_X0_imputation_dnorm_", Day1, "_", Day2, ".rds"))
  
} else {
  jm <- readRDS(paste0("RData/Fourier_X0_imputation_dnorm_", Day1, "_", Day2, ".rds"))
}
